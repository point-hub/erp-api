...
import { UpdateSalesQuotationRepository } from '../repositories/update.repository'
import { UpdateSalesQuotationUseCase } from '../use-cases/update.use-case'

export const updateSalesQuotationController: IController = async (controllerInput: IControllerInput) => {
  let session
  try {
    ...
    const updateSalesQuotationRepository = new UpdateSalesQuotationRepository(controllerInput.dbConnection)
    ...
    // 3.2 update
    const response = await UpdateSalesQuotationUseCase.handle(
      {
        auth: verifyTokenResponse as IAuth,
        _id: controllerInput.httpRequest.params.id,
        data: controllerInput.httpRequest.body,
      },
      { schemaValidation, updateSalesQuotationRepository },
    )
