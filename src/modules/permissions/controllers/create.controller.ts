...
import { CreatePermissionUseCase } from '../use-cases/create.use-case'

export const createPermissionController: IController = async (controllerInput: IControllerInput) => {
  let session
  try {
    ...
    // 2. define repository
    const createPermissionRepository = new CreatePermissionRepository(controllerInput.dbConnection)
    ...
    const response = await CreatePermissionUseCase.handle(
