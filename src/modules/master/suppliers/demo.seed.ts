...
export const seed = async (dbConnection: IDatabase, options: unknown) => {
  ...
  // prepare repository
  const createSupplierRepository = new CreateSupplierRepository(dbConnection)
  const retrieveAllSupplierGroupRepository = new RetrieveAllSupplierGroupRepository(dbConnection)
  ...
  const supplierGroups = await retrieveAllSupplierGroupRepository.handle({ page_size: 30 }, options)
  const counters = await retrieveAllCounterRepository.handle({ filter: { name: 'supplier_groups' } }, options)
  ...
  for (let index = 1; index <= 30; index++) {
    ...
    await createSupplierRepository.handle(seed, options)
