...
export const updateCustomerGroupController: IController = async (controllerInput: IControllerInput) => {
  ...
  try {
    ...
    // 2. define repository
    const updateCustomerGroupRepository = new UpdateCustomerGroupRepository(controllerInput.dbConnection, { session })
    ...
    const response = await UpdateCustomerGroupUseCase.handle(
      ...
      { schemaValidation, updateCustomerGroupRepository },
    )
