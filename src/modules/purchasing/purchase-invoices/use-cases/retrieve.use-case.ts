...
import { IBranch, IDetails, IRetrievePurchaseInvoiceRepository } from '../repositories/retrieve.repository'
...
export class RetrievePurchaseInvoiceUseCase {
  static async handle(input: IInput, deps: IDeps): Promise<IOutput> {
    // 1. database operation
    const response = await deps.retrievePurchaseInvoiceRepository.handle(input._id)
    // 2. output
    return {
      _id: response._id,
