...
export const seed = async (dbConnection: IDatabase, options: Record<string, unknown>) => {
  ...
  // prepare repository
  const createPurchaseInvoiceRepository = new CreatePurchaseInvoiceRepository(dbConnection, options)
  ...
  const counters = await retrieveAllCounterRepository.handle({ filter: { name: 'purchase_invoices' } })
  ...
  for (let index = 1; index <= 30; index++) {
    ...
    await createPurchaseInvoiceRepository.handle(seed)
