...
import { IBranch, IDetails, IRetrieveDownPaymentRepository } from '../repositories/retrieve.repository'
...
export class RetrieveDownPaymentUseCase {
  static async handle(input: IInput, deps: IDeps): Promise<IOutput> {
    // 1. database operation
    const response = await deps.retrieveDownPaymentRepository.handle(input._id)
    // 2. output
    return {
      _id: response._id,
