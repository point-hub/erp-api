...
import { CreateDownPaymentUseCase } from '../use-cases/create.use-case'

export const createDownPaymentController: IController = async (controllerInput: IControllerInput) => {
  let session
  try {
    ...
    // 3.2 create
    const response = await CreateDownPaymentUseCase.handle(
      {
