...
export const seed = async (dbConnection: IDatabase, options: Record<string, unknown>) => {
  ...
  // prepare repository
  const createDownPaymentRepository = new CreateDownPaymentRepository(dbConnection, options)
  ...
  const counters = await retrieveAllCounterRepository.handle({ filter: { name: 'down_payments' } })
  ...
  for (let index = 1; index <= 30; index++) {
    ...
    await createDownPaymentRepository.handle(seed)
