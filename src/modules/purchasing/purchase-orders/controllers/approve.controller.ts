...
import { ApprovePurchaseOrderRepository } from '../repositories/approve.repository'
import { ApprovePurchaseOrderUseCase } from '../use-cases/approve.use-case'

export const approvePurchaseOrderController: IController = async (controllerInput: IControllerInput) => {
  let session
  try {
    ...
    const approvePurchaseOrderRepository = new ApprovePurchaseOrderRepository(controllerInput.dbConnection)
    ...
    // 3.2 approve
    const response = await ApprovePurchaseOrderUseCase.handle(
      {
        auth: verifyTokenResponse as IAuth,
        _id: controllerInput.httpRequest.params.id,
      },
      { schemaValidation, approvePurchaseOrderRepository },
    )
