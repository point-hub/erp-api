...
export const seed = async (dbConnection: IDatabase, options: Record<string, unknown>) => {
  ...
  // prepare repository
  const createPurchaseRequestRepository = new CreatePurchaseRequestRepository(dbConnection, options)
  ...
  const counters = await retrieveAllCounterRepository.handle({ filter: { name: 'purchase_requests' } })
  ...
  for (let index = 1; index <= 30; index++) {
    ...
    await createPurchaseRequestRepository.handle(seed)
