...
export const seed = async (dbConnection: IDatabase, options: unknown) => {
  ...
  // prepare repository
  const createPurchaseRequestRepository = new CreatePurchaseRequestRepository(dbConnection)
  ...
  const counters = await retrieveAllCounterRepository.handle({ filter: { name: 'purchase_requests' } }, options)
  ...
  for (let index = 1; index <= 30; index++) {
    ...
    await createPurchaseRequestRepository.handle(seed, options)
