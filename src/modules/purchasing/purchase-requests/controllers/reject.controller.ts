...
import { RejectPurchaseRequestRepository } from '../repositories/reject.repository'
import { RejectPurchaseRequestUseCase } from '../use-cases/reject.use-case'

export const rejectPurchaseRequestController: IController = async (controllerInput: IControllerInput) => {
  let session
  try {
    ...
    const rejectPurchaseRequestRepository = new RejectPurchaseRequestRepository(controllerInput.dbConnection)
    ...
    // 3.2 reject
    const response = await RejectPurchaseRequestUseCase.handle(
      {
