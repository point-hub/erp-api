...
import { CreateReceiveOrderUseCase } from '../use-cases/create.use-case'

export const createReceiveOrderController: IController = async (controllerInput: IControllerInput) => {
  let session
  try {
    ...
    // 3.2 create
    const response = await CreateReceiveOrderUseCase.handle(
      {
