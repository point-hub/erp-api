...
import { DeleteReceiveOrderRepository } from '../repositories/delete.repository'
import { RetrieveReceiveOrderRepository } from '../repositories/retrieve.repository'
import { DeleteReceiveOrderUseCase } from '../use-cases/delete.use-case'

export const deleteReceiveOrderController: IController = async (controllerInput: IControllerInput) => {
  let session
  try {
    ...
    const deleteReceiveOrderRepository = new DeleteReceiveOrderRepository(controllerInput.dbConnection, {
      session,
    })
    ...
    // 3.2 delete
    const response = await DeleteReceiveOrderUseCase.handle(
      {
