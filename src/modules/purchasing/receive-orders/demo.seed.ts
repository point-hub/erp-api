...
export const seed = async (dbConnection: IDatabase, options: Record<string, unknown>) => {
  ...
  // prepare repository
  const createReceiveOrderRepository = new CreateReceiveOrderRepository(dbConnection, options)
  ...
  const counters = await retrieveAllCounterRepository.handle({ filter: { name: 'receive_orders' } })
  ...
  for (let index = 1; index <= 30; index++) {
    ...
    await createReceiveOrderRepository.handle(seed)
