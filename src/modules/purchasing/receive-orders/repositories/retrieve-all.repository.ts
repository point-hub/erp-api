...
export class RetrieveAllReceiveOrderRepository implements IRetrieveAllReceiveOrderRepository {
  ...
  private aggregateFilters(auth: IAuth, query: IQuery) {
    ...
    if (query.filter?.search) {
      ...
      filtersOr.push({ driver: { $regex: query.filter?.search, $options: 'i' } })
      filtersOr.push({ license_plate: { $regex: query.filter?.search, $options: 'i' } })
    ...
    if (query.filter?.has_invoice) filtersAnd.push({ has_invoice: { $exists: false } })
